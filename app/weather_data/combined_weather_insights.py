...
class OttawaCombinedProductivity(): 
    ...
    def generate_work_capacity_insights(self, i =1): 
        if i in [11, 12, 1, 2, 3, 4]: 
            ...
            '75m work period with 2 breaks / 4hr work block' : (monthly_data['extremes'].between(-26, -20) & monthly_data['Wind Spd (km/h)'].between(16,24)),
            '55m work period with 3 breaks / 4hr work block' : (monthly_data['extremes'].between(-26, -20) & monthly_data['Wind Spd (km/h)'].between(24,32)),
            '40m work period with 4 breaks / 4hr work block' : (monthly_data['extremes'].between(-26, -20) & monthly_data['Wind Spd (km/h)'].round() > 32), 
            }
            ...
            humidity_rules = {
            '75m work period with 2 breaks / 4hr work block' : (monthly_data['extremes'].between(25, 30)) ,
            '55m work period with 3 breaks / 4hr work block' : (monthly_data['extremes'].between(30, 35)) ,
            '40m work period with 4 breaks / 4hr work block' : (monthly_data['extremes'] > 35)
        }

            monthly_data['humid_conditions'] = np.select(humidity_rules.values(), humidity_rules.keys())
    ...
    def make_productivity_curve(self): 
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=self.x, y=self.polyval, fill='tozeroy')) 
        fig.update_layout(
            title={
        ...
        self.prductivity_figure = fig.show(auto_open = False)
    ...
    def plot_productivity_by_month(self, i = 1):
        ...
        y_poly_fit = np.polyfit(fit_x, fit_y, deg = 20)
        ...
        self.plot_productivity_plot = fig.show()
