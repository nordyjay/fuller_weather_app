...
class OttawaWeatherAnalytics():
    ...
    def __init__(self, data, smoothing_amount = 1): 
        ...
        self.month_list = ['january', 'february', 'march', 'april', 'may', 
                      ...
                      'november', 'december'] 
        self.month_dict = {0: 'january',1: 'february',2: 'march',3: 'april',4: 'may',
        ...
        self.productivity_labels = [.25, .4, .56, .8, 1, .37, .35, .2]
    ...
    def prepare_daily_temp_stats(self): 
        self.daily_temp_stats = self.daily_data.agg({'Temp (°C)': ['mean', 'max', 'min', 'sem']})
        self.daily_temp_stats = self.daily_temp_stats.droplevel(0, axis = 1)
        self.daily_temp_stats = self.daily_temp_stats.rename(columns = {'mean': 'temp_mean',
                                                                        'max' : 'temp_max',
                                                                        'min' : 'temp_min',  
    ...
    def prepare_daily_wind_chill_stats(self): 
        self.daily_wind_chill_stats = self.daily_data.agg({'Wind Chill': ['mean', 'min', 'sem']})
        self.daily_wind_chill_stats = self.daily_wind_chill_stats.droplevel(0, axis = 1)
        self.daily_wind_chill_stats = self.daily_wind_chill_stats.rename(columns = {'mean': 'wind_chill_mean',
                                                                                    'min' : 'wind_chill_max', 
    ...
    def prepare_daily_humidity_stats(self): 
        self.daily_humidity_stats = self.daily_data.agg({'Hmdx': ['mean', 'max', 'sem']})
        self.daily_humidity_stats = self.daily_humidity_stats.droplevel(0, axis = 1)
        self.daily_humidity_stats = self.daily_humidity_stats.rename(columns = {'mean': 'humidity_mean',
                                                                                'max' : 'humidity_max',  
    ...
    def daily_master_stats(self): 
        self.daily_master_stats = self.daily_temp_stats.join(self.daily_wind_chill_stats, how = 'left').join(self.daily_humidity_stats, how = 'left')
        self.daily_master_stats['extremes'] = self.daily_master_stats['wind_chill_max']  + self.daily_master_stats['humidity_max']
        self.daily_master_stats['extremes_smoothing'] = self.daily_master_stats['extremes'].ewm(halflife = self.smoothing_amount).mean()
        self.daily_master_stats['cutting'] = pd.to_numeric(pd.cut(self.daily_master_stats['extremes_smoothing'],
                        bins = self.temperature_bins, 
                        labels = self.productivity_labels))
        self.productivity_value_counts = pd.DataFrame(self.daily_master_stats['cutting'].value_counts() / len(self.daily_master_stats.index.year))
        
        
        return self.daily_master_stats, self.productivity_value_counts
        
    def prepare_monthly_master_stats(self):
        self.master_monthly_stats = self.daily_master_stats[['extremes']].groupby([self.daily_master_stats.index.month, self.daily_master_stats.index.day]).agg('mean')
        self.master_monthly_stats = self.master_monthly_stats.unstack(level = 0).droplevel(1, axis = 1)
        self.master_monthly_stats.columns = self.month_list
        for m in self.master_monthly_stats: 
            self.monthly_productivity_df[m] = self.master_monthly_stats[m].apply(lambda x: np.polyval(self.polyfit, x))
        for m in self.month_list: 
            df = pd.concat([self.master_monthly_stats[[m]], self.monthly_productivity_df.ewm(halflife = 10).mean()[[m]]], axis = 1)
            df.columns = [f'{m}_temp', f'{m}_productivity']
            self.temp_productivity_frames.append(df)
            
        
        return self.master_monthly_stats, self.monthly_productivity_df, self.temp_productivity_frames
    ...
    def make_monthly_productivity_plot(self): 
        self.fig2 = go.Figure()
        self.fig2.add_trace(go.Scatter(x=self.month_list, y=self.monthly_productivity_df.mean(), fill='tozeroy')) 
    ...
    def plot_productivity_by_month(self, i = 1): 
        ...
        self.fig3 = go.Figure()
        self.fig3.add_trace(go.Scatter(x=self.temp_productivity_frames[self.tdi[i][0]].index, 
                                 y=self.temp_productivity_frames[self.tdi[i][0]].iloc[:, 1], 
                                 fill='tonexty', line_color = 'hotpink',))
